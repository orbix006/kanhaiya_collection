import { getAboutSectionsAdmin } from "@/lib/data/cms";
...
export default async function AdminAboutPage() {
  const sections = await getAboutSectionsAdmin();
      ...
      <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4 pb-6 border-b border-border/60">
        <div>
          ...
          <h1 className="text-2xl sm:text-3xl font-extrabold tracking-tight text-foreground">
            About Us Sections & Storytelling
          </h1>
          <p className="text-xs sm:text-sm text-muted-foreground mt-1 max-w-2xl">
            Add, reorder, and illustrate modular story blocks on the public About Us page to highlight heritage, craftsmanship, and sustainability.
          </p>
        </div>
        ...
        <div className="flex items-center gap-2 text-xs text-muted-foreground bg-muted/60 px-3 py-1.5 rounded-xl border border-border">
          ...
          <span>{sections.length} story sections</span>
        </div>
      </div>

      <AboutManager initialSections={sections} />
    </div>
  );
}
